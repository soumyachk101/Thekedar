...
5. **Self-check** every acceptance checkbox; consult `.thekedar/knowledge/pitfalls/python.md` for the specific traps before reporting done.
...
- The traps in `.thekedar/knowledge/pitfalls/python.md` are law here: no Py2-isms, no mutable default args, `is` only for `None`/identity.
...
- Never invent APIs, stdlib functions, or packages — Grep/Read or check the manifest to verify. Uncertainty = check, not guess (`.thekedar/knowledge/pitfalls/python.md`).
...
- Parameterized queries only; never string-build SQL (`.thekedar/knowledge/security/owasp/a03-injection.md`).
