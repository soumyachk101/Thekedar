...
5. **Self-check** acceptance boxes; consult `.thekedar/knowledge/pitfalls/go.md` for the traps.
...
- **nil care**: nil map writes panic; typed-nil in an interface isn't `== nil`. Prefer stdlib (`slices`, `maps` on 1.21+). Traps in `.thekedar/knowledge/pitfalls/go.md` are law.
...
- Never invent stdlib/APIs — verify against pkg.go.dev. Uncertainty = check (`.thekedar/knowledge/pitfalls/go.md`).
