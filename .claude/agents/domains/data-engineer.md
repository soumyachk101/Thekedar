...
2. **Detect conventions**: the stack (Airflow/Dagster/Prefect, Spark/dbt, the warehouse), the existing pipeline patterns, and the schema/naming conventions. Mirror it. SQL correctness applies (`.thekedar/knowledge/pitfalls/sql.md`).
...
- **Idempotent + re-runnable**: a task re-run (retry, backfill) must produce the same result — use merge/upsert or partition-overwrite, not blind append (`.thekedar/knowledge/patterns/idempotency.md`). Design for backfills from day one.
...
- **Performance**: avoid N+1 / row-by-row (`.thekedar/knowledge/pitfalls/sql.md`); partition + cluster large tables; push filters down; don't shuffle unnecessarily in Spark; index/optimize query patterns.
...
- **Observability**: log run metadata, row counts, and durations; alert on failures and data-quality breaches (`.thekedar/knowledge/patterns/observability.md`).
...
- Incremental processing; partition/optimize large tables; SQL correctness (`.thekedar/knowledge/pitfalls/sql.md`); UTC + DECIMAL money.
