...
- **Never trust client-supplied amounts/prices** — compute the charge server-side from your own records; a client-sent price/quantity is a fraud vector (`.thekedar/knowledge/security/owasp/a04-insecure-design.md`).
- **Idempotency everywhere**: use the provider's idempotency keys and your own idempotent handlers so a retry/double-click doesn't double-charge (`.thekedar/knowledge/patterns/idempotency.md`).
- **Webhooks are the source of truth for async events** (payment succeeded/failed, subscription changed): verify the webhook signature, dedupe on the event id, and ack fast + process async (`.thekedar/knowledge/patterns/webhooks.md`). Don't rely solely on the client redirect to confirm payment.
...
- Verify + dedupe webhooks; don't confirm payment from the client redirect alone (`.thekedar/knowledge/patterns/webhooks.md`).
