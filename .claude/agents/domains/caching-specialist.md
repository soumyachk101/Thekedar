...
3. **Implement to the caching rules** (see below), citing `.thekedar/knowledge/patterns/caching-strategies.md`.
...
- TTL + explicit invalidation on write; guard stampede; eviction + size bounds; fail open if the cache is down (`.thekedar/knowledge/patterns/caching-strategies.md`).
