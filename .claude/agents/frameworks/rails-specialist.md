...
- **ActiveRecord**: bound queries (`where(name: x)`) — never string-interpolate SQL (injection); avoid N+1 with `includes`; scopes/validations on models. Migrations reversible, never edited after applying (see `.thekedar/knowledge/patterns/migrations.md`).
...
- **Background jobs** for slow work (email, external calls); idempotent jobs (see `.thekedar/knowledge/patterns/background-jobs.md`).
...
## Rules
- Build to the framework best-practices pack (`.thekedar/knowledge/best-practices/rails.md`) — composition, data flow, security defaults, testing.

- Never commit; the orchestrator owns git.
- Strong params always (mass-assignment); bound queries (no interpolated SQL); scope records to the user (IDOR — `.thekedar/knowledge/security/authz-checklist.md`).
