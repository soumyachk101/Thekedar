...
5. **Self-check** acceptance boxes; consult `.thekedar/knowledge/pitfalls/typescript-javascript.md`, `.thekedar/knowledge/patterns/api-design.md`.
...
- **Async/DB**: await everything; parameterized queries via the ORM; correct status codes + consistent error shape (`.thekedar/knowledge/pitfalls/api-http.md`).
...
- Constructor DI (no `new` services); DTOs + ValidationPipe for input; Guards for authz (`.thekedar/knowledge/security/authz-checklist.md`).
