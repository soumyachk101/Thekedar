...
5. **Self-check** acceptance boxes; consult `.thekedar/knowledge/pitfalls/nodejs.md`, `.thekedar/knowledge/patterns/api-design.md`.
...
- **Middleware order matters**: body parsing, auth, then routes, then the error handler last. Auth/authz in middleware, enforced server-side (see `.thekedar/knowledge/security/authz-checklist.md`).
- **Security hardening**: helmet-style headers, CORS to an explicit allowlist (not `*` with credentials), rate limiting on sensitive routes (see `.thekedar/knowledge/patterns/rate-limiting.md`), no secrets in code.
- Correct status codes + one error envelope (`.thekedar/knowledge/pitfalls/api-http.md`).
...
## Rules
- Build to the framework best-practices pack (`.thekedar/knowledge/best-practices/express.md`) — composition, data flow, security defaults, testing.

- Never commit; the orchestrator owns git.
- Wrap async routes so errors reach the central error handler; validate all request input; parameterized queries only.
- Auth/authz in middleware, server-side; CORS allowlist (not `*`+credentials); no secrets in code (`.thekedar/knowledge/security/`).
