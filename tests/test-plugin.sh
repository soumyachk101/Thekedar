#!/usr/bin/env bash
# test-plugin.sh — plugin manifests are valid and internally consistent:
# JSON parses, declared component paths exist, hooks.json wires real hook
# scripts, and versions agree with VERSION.
set -u
HERE="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
ROOT="$(cd "$HERE/.." && pwd)"
PDIR="$ROOT/.claude-plugin"
CDIR="$ROOT/.codex-plugin"
...
# 1. Claude plugin manifests are valid JSON
...
for f in "$CDIR/plugin.json" "$ROOT/.codex/hooks/hooks.json" "$ROOT/marketplace.json"; do
  if [ -f "$f" ] && valid_json "$f"; then ok "${f#"$ROOT"/} is valid JSON"; else bad "$f missing or invalid JSON"; fi
done
...
cname="$(jget "$CDIR/plugin.json" "d.get('name','')" ".name")"
cver="$(jget "$CDIR/plugin.json" "d.get('version','')" ".version")"
if [ "$cname" = "thekedar" ]; then ok "Codex plugin name is 'thekedar'"; else bad "Codex plugin name '$cname' != thekedar"; fi
if [ "$cver" = "$fver" ]; then ok "Codex plugin version ($cver) matches VERSION ($fver)"; else bad "Codex plugin version '$cver' != VERSION '$fver'"; fi
...
if [ -f "$ROOT/.codex/hooks/hooks.json" ]; then ok "Codex hooks file present"; else bad "Codex hooks.json missing"; fi
...
for ev,arr in d.items():
    for m in arr:
        for h in m.get("hooks",[]):
            print(h.get("command",""))
' "$ROOT/.codex/hooks/hooks.json" 2>/dev/null)"
else
  ccmds="$(jq -r '.hooks[][]?.hooks[]?.command // empty' "$ROOT/.codex/hooks/hooks.json" 2>/dev/null)"
fi
while IFS= read -r c; do
  [ -z "$c" ] && continue
  script="$(printf '%s' "$c" | sed -n 's/.*hooks\/\([a-z-]*\.sh\).*/\1/p')"
  ...
  if [ -f "$ROOT/hooks/$script" ]; then ok "Codex hooks.json -> hooks/$script exists"; else bad "Codex hooks.json references missing hooks/$script"; fi
done <<EOF
$ccmds
EOF
...
for h in session-brief scope-guard secret-guard munshi; do
  ...
  if grep -q "$h.sh" "$ROOT/.codex/hooks/hooks.json"; then ok "wired in Codex hooks: $h"; else bad "Codex hooks.json missing $h"; fi
done

# 6. marketplaces list the plugin with source "./"
...
cmsrc="$(jget "$ROOT/marketplace.json" "d['plugins'][0].get('source',{}).get('path','')" ".plugins[0].source.path")"
if [ "$cmsrc" = "./" ]; then ok "Codex marketplace plugin source path is './'"; else bad "Codex marketplace source path '$cmsrc' != './'"; fi
