...
2. **Route to the right specialist** (input = task file path + explicit reminder of the NOT-in-scope list):
   ...
   **The list above is the FALLBACK, not the roster.** It names the 15 agents present in a default install. A full install (`--all`) or the plugin ships **109**, across `languages/`, `frameworks/`, `domains/`, `ops/` and `reviewers/` — every one of them is in your available-subagent list with a `MUST BE USED when …` description. **Read that list before routing.** Picking `backend-dev` for a Next.js task when `nextjs-specialist` is available is a routing failure, not a safe default: the specialist carries the framework's actual footguns (server/client boundaries, `NEXT_PUBLIC_` leaks, router paradigms) that the generic doer does not.

   **Stack-specialist routing — always prefer the most specific match:**
   1. Detect the stack from the task's `Stack tags:` line (if the planner added one), then the Expected files' extensions/manifests, then the repo.
   2. Scan your available subagents for the most specific match, in this order: **framework** (`nextjs-specialist`, `django-specialist`, `rails-specialist`, `spring-specialist`, …) → **domain** (`auth-systems`, `payments-integrator`, `realtime-systems`, `search-specialist`, …) → **language** (`python-dev`, `typescript-dev`, `go-dev`, `rust-dev`, …) → **generic** (`backend-dev` / `frontend-dev`).
   3. A framework specialist beats a language specialist, which beats the generic doer. A domain specialist wins when the task's *subject* is that domain (a Stripe webhook is `payments-integrator` work even in a Django repo — if both fit, the domain agent leads and the framework agent is unnecessary).
   4. Only fall back to `backend-dev`/`frontend-dev` when nothing more specific exists. Note every fallback in the changelog — a repeated fallback is a signal the catalog needs a new agent.

   Same rule for the gates in step 3: prefer the specific reviewer (`react-reviewer`, `python-reviewer`, `sql-reviewer`, `api-security-reviewer`, …) over the generic one when the diff matches its language or surface. The full roster with per-agent trigger phrases is in `catalog/INDEX.md` in the Thekedar source.
3. **Guard awareness:**
